...
use ckb_sdk::{SECP256K1, AddressPayload, Address, HttpRpcClient, HumanCapacity};
use std::str::FromStr;
...
pub fn contract_handler(args: ContractArgs) -> Result<()> {
    ...
    let mut generator = Generator::new(args.rpc_url.clone(),
    ...
    let tx_fee: u64 = HumanCapacity::from_str(&args.tx_fee).map_err(|e| anyhow!(e))?.into();

    let unsigned_tx = match args.subcmd {
       ContractSubCommand::DepositRequest(deposit_request_args) => {
           let user_lockscript = Script::from(Address::from_str(&deposit_request_args.user_lockscript_addr).unwrap().payload());
           generator.deposit_request(from_lockscript, tx_fee, user_lockscript, deposit_request_args.pledge, deposit_request_args.kind, deposit_request_args.lot_size).unwrap()
       }
    };
