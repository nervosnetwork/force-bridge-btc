...
fn generate_btc_correct_case() -> TestCase {
    ...
    let tockb_data = ToCKBCellDataTest {
        ...
        x_extra: XExtraView::Btc(BtcExtraView {
            lock_tx_hash: hex::decode(clear_0x(
                "0x5227c5fbad9d9202ade7f02452cf880dac1ed270255ebfe6716e8b3e8956571d",
            ))
            .unwrap()
            .into(),
            lock_vout_index: 1,
    ...
    let case = TestCase {
        kind,
        input_status: Some(ToCKBStatus::Warranty as u8),
        output_status: Some(ToCKBStatus::FaultyWhenWarranty as u8),
...
fn test_wrong_lot_size() {
    ...
    case.output_tockb_cell_data.lot_size = 200;
    case.expect_return_code = InvariantDataMutated as i8;
    run_test_case(case);
}

#[test]
fn test_wrong_capacity() {
    let mut case = generate_btc_correct_case();
    case.input_capacity = 100;
    case.output_capacity = 200;
