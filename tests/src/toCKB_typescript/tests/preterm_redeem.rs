...
#[test]
fn test_correct_tx_when_deposit_requestor_is_redeem() {
    let correct_eth_address = basic::Bytes::new_builder()
        .set([Byte::new(1u8); 20].to_vec())
        .build();
    ...
    let always_success_lockscript = context
        ...
        .expect("script");
    let redeemer_script = basic::Script::from_slice(always_success_lockscript.as_slice()).unwrap();
    let toCKB_data = build_to_ckb_data(
        1,
        Script::new_builder().build(),
        correct_eth_address.clone(),
...
#[test]
fn test_wrong_tx_when_deposit_requestor_is_redeem_but_output_xt_not_none() {
    let correct_eth_address = basic::Bytes::new_builder()
        .set([Byte::new(1u8); 20].to_vec())
        .build();
    let mut context = Context::default();
